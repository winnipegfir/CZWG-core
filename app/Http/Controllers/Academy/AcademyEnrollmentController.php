...
class AcademyEnrollmentController extends Controller
{
    public function index()
    {
        ...
        $homeMembers = VatcanMember::where('active_home_member', true)
            ->orderBy('rating_id')->orderBy('last_name')->orderBy('first_name')->get();
        $visitorMembers = VatcanMember::where('active_visitor_member', true)
            ->orderBy('rating_id')->orderBy('last_name')->orderBy('first_name')->get();
        ...
        return view('academy.admin.enrollments', compact(
            'courses', 'students', 'enrollments', 'homeMembers', 'visitorMembers', 'lastSync',
    ...
    public function store(Request $request)
    {
        ...
        abort_if($course->visitor_only && ! AcademyAccess::isQualifiedVisitor($user), 422, 'Visitor Courses can only be assigned to an eligible S3-or-higher visiting controller.');
    ...
    public function syncVatcan()
    {
        ...
        return back()->with('success', sprintf(
            'VATCAN sync complete: %d home controllers, %d visitors, %d matched accounts, %d pending CIDs, %d enrollments activated.',
